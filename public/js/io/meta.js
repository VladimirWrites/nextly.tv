...
/* ---- movies ----
   ...
   Which catalogue answers for a movie follows the one that has been chosen, the same as
   everything else — it just cannot be TVmaze, which has no movies at all.
   ...
   chosen TVmaze still had every movie come from TMDB and be stored under a TMDB key, which is
   ...
   The extras a movie page asks for on top follow it too. Cinemeta has no people and no
...
/* The people in a movie, and what to watch next after it. Both are TMDB only, and both return
   ...
   behind them, so there is nobody to open. A movie from there shows its cast as text.
...
export function movieCredits(m) {
  ...
  return once(`movie-cast:${movieProvider().id}:${m.key || m.id}`, async () => {
    ...
    // A movie held under Cinemeta's key is still a movie TMDB knows, by its IMDb id.
...
/* One movie, from whichever catalogue its key names, falling back the way shows do: a key whose
   catalogue cannot be reached is still a movie with an IMDb id, and the other one can place it. */
export async function fetchMovie(key, { imdb = null } = {}) {
  ...
  /* Its own catalogue cannot answer — the key was removed, or the service is gone. The movie is
     still a movie, and the vault holds an IMDb id for it, which is the whole reason portable ids
  ...
  throw new Error(`${(own || cinemeta).label} can't be reached, and this movie has no id to find it by elsewhere.`);
}
